...
document.addEventListener('DOMContentLoaded', () => {
    ...
    filterByCategory.addEventListener('change', (event) => {
        ...
        const sortedProducts = sortProducts(filteredProducts, sortByPrice.value ? 'price' : 'popularity', sortByPrice.value ? sortByPrice.value.split('-')[0] : sortByPopularity.value.split('-')[0]);
        displayProducts(sortedProducts);
    });

    sortByPrice.addEventListener('change', (event) => {
        const sortOrder = event.target.value;
        const sortedProducts = sortProducts(filterProducts(filterByCategory.value), 'price', sortOrder.split('-')[0]);
        displayProducts(sortedProducts);
    });

    sortByPopularity.addEventListener('change', (event) => {
        const sortOrder = event.target.value;
        const sortedProducts = sortProducts(filterProducts(filterByCategory.value), 'popularity', sortOrder.split('-')[0]);
        displayProducts(sortedProducts);
    });
